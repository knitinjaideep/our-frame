...
def get_album_detail(
    ...
    album_summary = _to_album_summary_with_resolved_cover(session, album) if album else AlbumSummary(
...
def _photo_in_album_tree(session: Session, album_id: str, photo: DrivePhoto) -> bool:
    """
    ...
    for _ in range(_COVER_TREE_MAX_DEPTH):
        ...
        for folder_id in frontier:
            for child in album_repo.get_by_parent(session, folder_id):
                if child.id in seen:
                    continue
                if child.id == parent:
                    return True
...
def set_album_cover(session: Session, album_id: str, photo_id: str | None) -> AlbumSummary:
    """
    ...
    if photo_id is not None:
        photo = photo_repo.get_by_id(session, photo_id)
        if not photo:
        ...
        if not _photo_in_album_tree(session, album_id, photo):
            raise PhotoNotInAlbumError(photo_id)
    updated = album_repo.set_cover_photo(session, album_id, photo_id)
    return _to_album_summary_with_resolved_cover(session, updated)
