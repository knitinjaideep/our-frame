...
def _to_response(session: Session, fav: Favorite, mime_type: str = "image/jpeg") -> FavoriteResponse:
...
def list_favorites(session: Session) -> FavoritesListResponse:
    ...
    for fav in favs:
        ...
        responses.append(_to_response(session, fav, mime))
...
def add_favorite(session: Session, body: FavoriteCreate) -> FavoriteResponse:
    ...
    photo = photo_repo.get_by_id(session, body.photo_id)
    mime = photo.mime_type if photo and photo.mime_type else "image/jpeg"
    if existing:
        return _to_response(session, existing, mime)
    fav = favorites_repo.add(session, body.photo_id, body.photo_name, body.folder_id)
    return _to_response(session, fav, mime)
